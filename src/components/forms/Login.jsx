        ...
        <div className='relative bg-opacity-50 z-10  w-full h-full  bg-[url(https://assets.nflxext.com/ffe/siteui/vlv3/a73c4363-1dcd-4719-b3b1-3725418fd91d/fe1147dd-78be-44aa-a0e5-2d2994305a13/IN-en-20231016-popsignuptwoweeks-perspective_alpha_website_large.jpg)] bg-cover'>
            ...
            <div className="z-50 bg-black mx-auto sm:w-[400px] bg-opacity-75 text-white mb-20 px-16 py-10 rounded-lg">
                <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
                    {({ isSubmitting }) => (
                        <Form>
                            <h2 className='text-3xl font-bold mb-7'>{isSignInForm ? "Sign In" : "Sign Up"}</h2>
                            {!isSignInForm && (
                                <div className="mb-7">
                                    <Field type="name" name="name" placeholder="Your Name" className="w-full px-3 py-3 rounded-md bg-[#333333] outline-none" />
                                    <ErrorMessage name="name" component="div" className="text-red-500 text-sm" />
                                </div>
                            )}
                            <div className="mb-7">
                                <Field type="email" name="email" placeholder="Email or phone number" className="w-full px-3 py-3 rounded-md bg-[#333333] outline-none" />
                                <ErrorMessage name="email" component="div" className="text-red-500 text-sm" />
                            </div>
                            <div className="mb-7">
                                <Field type="password" name="password" placeholder="Password" className="w-full px-3 py-3 rounded-md bg-[#333333] outline-none" />
                                <ErrorMessage name="password" component="div" className="text-red-500 text-sm" />
                            </div>
                            <div className="flex justify-center mb-2">
                                <button type="submit" disabled={isSubmitting} className="bg-[#e50815] w-full py-3 rounded-md">
                                    {isSubmitting ? "Submitting..." : isSignInForm ? "Sign In" : "Sign Up"}
                                </button>
                            </div>
                            <div className="mb-10">
                            ...
                            {isSignInForm ? (
                                ...
                                <div className="mb-5" onClick={toggleSignInForm}>
                                    <span className="font-light text-gray-400">Already registered?</span>
                                    <Link to='/'>
                                        <span className="font-light hover:underline">Sign In Now?</span>
                                    </Link>
                                </div>
                            )}
                            <div>
